...
class FastReviewer:
    ...
    def review(self, draft_text: str) -> Optional[Dict[str, Any]]:
        ...
        return self._build_review(primary, analysis["text"], analysis["risk_level"])
    ...
    def _build_review(
        ...
        if style == "progress":
            return {
                "risk_level": risk_level,
                "scene": primary.scene,
                "problem": "只丢压力和截止时间，没有说清楚优先级、卡点怎么处理、需要什么支持。对方只感到被压，不敢主动暴露风险，短期也许先答应，长期更容易拖到最后才同步真实问题。",
                "rewrite_mild": f"这项需要在{deadline}前完成，完成后发我确认一下。如果现在有阻塞点，随时同步具体问题和预计完成时间，我来一起想办法。",
                "rewrite_balanced": f"这项需要在{deadline}前完成并发我确认。如果现在有阻塞点，请直接同步具体问题、影响范围和预计完成时间，我来一起判断怎么处理。",
                "rewrite_direct": f"这项我现在提到最高优先级，请在{deadline}前完成并发我。若有阻塞，马上同步具体问题和需要的支持，不要拖到最后。",
                "follow_up": "如果这真是硬截止，最好补一句为什么必须在这个时间点前完成，以及哪些事项可以顺延。",
            }
        ...
        if style == "feedback":
            return {
                "risk_level": risk_level,
                "scene": primary.scene,
                "problem": "把反馈和分歧说成了人格、态度或成熟度问题，用抽象标签代替具体事实。对方会更在意“你是不是在否定我这个人”，而不是具体该改什么，结果就是防御上升、执行性下降。",
                "rewrite_mild": "这次输出有几个具体点我觉得可以再调一下，不是说整体有问题。我们一起看看事实和差距，商量下一步怎么改。",
                "rewrite_balanced": "我想指出的不是你这个人有问题，而是这次输出里有几个具体点没达标。我们直接对齐事实、差距和下一步怎么改。",
                "rewrite_direct": "这次结果还不能过，问题在具体输出而不在态度标签。我们现在把缺的点列清楚，按标准改到位。",
                "follow_up": "最好紧接着补上 2 到 3 个可观察的问题点、对应标准和复查时间，不要停留在抽象评价上。",
            }
        ...
        if style == "moralizing":
            return {
                "risk_level": risk_level,
                "scene": primary.scene,
                "problem": "用关系、感恩或“为你好”替代清晰的业务边界和任务说明，把工作要求包装成情感债。对方会觉得你在用情感债或关系亲近感压缩讨论空间，短期也许更顺从，但长期的信任和边界感都会受损。",
                "rewrite_mild": "我想把这次的要求说得更具体一些：需要你支持的任务、范围和时间分别是什么，我们一起对齐一下。",
                "rewrite_balanced": "我说这件事不是为了上价值，而是想把要求说清楚：这次需要你支持的具体任务、范围、截止时间和协作方式分别是什么。",
                "rewrite_direct": "这件事按工作要求推进就行，我们把任务范围、截止时间和责任边界讲清楚，不需要再上升到关系、感恩或态度。",
                "follow_up": "把“为你好”“一家人”“要懂感恩”这类说法换成具体要求、边界和支持方式，会更稳。",
            }
        ...
        if style == "meeting":
            return {
                "risk_level": risk_level,
                "scene": primary.scene,
                "problem": "把纠正和施压放进了公开场合，容易从对事变成示众式羞辱。公开点名会让人先保脸面，再考虑问题本身；短期可能服软，长期更容易沉默、甩锅或回避责任。",
                "rewrite_mild": "这个问题我们会后单独聊一下。你先把当前情况和想法整理一下发我，我们对齐后再看怎么推进。",
                "rewrite_balanced": "这个问题我先和你单独对一下。请你在会后把当前情况、风险和解决方案发我，我们对齐后再同步团队。",
                "rewrite_direct": "这个问题需要尽快收口，但不适合在公开场合上升到人身。会后请你把现状、风险和方案发我，我来拍板下一步。",
                "follow_up": "如果必须在会上处理，也尽量只讲事实、影响和 owner，不做横向羞辱和人格评价。",
            }
        ...
        if style == "boundary":
            return {
                "risk_level": risk_level,
                "scene": primary.scene,
                "problem": "用忠诚、吃苦或抗压包装额外索取，边界和安排都不够清楚。对方会觉得你在用态度和投入感压过实际负荷，表面也许会答应，心里却更容易积累抵触和不信任。",
                "rewrite_mild": "这项现在确实比较急，我先把情况说一下：大概需要额外投入多久、为什么现在比较紧，其他事项我也会帮忙协调。",
                "rewrite_balanced": "这项现在确实比较急，我先把要求说清楚：需要额外投入多久、为什么必须现在做、其他优先级怎么调整，我也会一起协调资源。",
                "rewrite_direct": "这项今天需要加急，我会明确优先级和持续时间。请你先按这个安排执行；如果负荷冲突，马上同步，我来取舍。",
                "follow_up": "涉及加班、周末或持续高压时，最好把时间范围、补偿方式和优先级调整明确说出来。",
            }

        return {
            "risk_level": risk_level,
            "scene": primary.scene,
            "problem": "用资历、服从或“我来替你决定”压过事实讨论和对方的判断空间。对方会感觉自己没有表达分歧和风险的空间，团队容易表面服从、私下失真，真实问题也更难被提早暴露。",
            "rewrite_mild": "这里我先说一下我的判断和依据，你也把你的想法和顾虑说出来，我们一起看看怎么定。",
            "rewrite_balanced": "这里我先把当前约束和判断依据说清楚。你也把分歧点和风险说出来，我们在这个边界内定最后方案。",
            "rewrite_direct": "这件事现在需要统一执行，我来承担拍板责任。但执行前请把关键风险说清楚，避免因为信息被压住而返工。",
            "follow_up": "如果必须拍板，就把原因、边界和复盘点讲明白，而不是只强调身份、年资或服从。",
        }
